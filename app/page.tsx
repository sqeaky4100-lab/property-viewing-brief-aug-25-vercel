...
export default function Home() {
  ...
  const callHref = agentPhone ? `tel:${agentPhone}` : undefined;
  const smsHref = agentPhone ? `sms:${agentPhone}` : undefined;
  useEffect(() => {
    const saved = localStorage.getItem("pvb-aug25-checklist");
    if (!saved) return;
    try {
      const parsed: unknown = JSON.parse(saved);
      if (Array.isArray(parsed) && parsed.every((item) => typeof item === "string")) {
        queueMicrotask(() => setChecked(parsed));
      }
    } catch {
      localStorage.removeItem("pvb-aug25-checklist");
    }
  }, []);
  ...
  return <main className="min-h-screen pb-[calc(6.5rem+env(safe-area-inset-bottom))]">
    <header className="sticky top-0 z-40 border-b border-border bg-background/88 pt-[env(safe-area-inset-top)] backdrop-blur-xl"><div className="mx-auto flex max-w-2xl items-center gap-2 px-3 py-2">
      <Sheet><SheetTrigger asChild><Button variant="outline" size="icon" aria-label="Open tour navigation"><Menu className="size-5" /></Button></SheetTrigger><SheetContent>
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-6"><div className="mb-3 rounded-2xl bg-amber-500/[.12] p-3 text-sm"><p className="font-bold">Cathy is coordinating access</p><p className="mt-1 text-xs text-muted-foreground">Belleview is likely appointment-dependent. Start there if confirmed.</p><Button asChild variant="heat" className="mt-3 w-full"><a href={callHref ?? "#"} aria-disabled={!callHref} onClick={(event) => { if (!callHref) event.preventDefault(); }}><Phone className="size-4" />Call Cathy</a></Button></div>
    ...
    <div className="mx-auto max-w-2xl px-3">
      ...
      <Card className="mb-3 border-amber-500/35 bg-amber-500/[.09]"><CardContent className="pt-4 text-center"><p className="flex items-center justify-center gap-2 font-bold"><CalendarClock className="size-5 text-amber-700 dark:text-amber-300" />Tuesday afternoon · exact time not in email</p><p className="mx-auto mt-1 max-w-lg text-[15px] leading-5 text-muted-foreground">Cathy: 2 vacant, 1 appointment. Put Belleview first if that is the scheduled home.</p><div className="mx-auto mt-4 grid max-w-sm grid-cols-2 gap-2"><Button className="min-h-12 w-full rounded-2xl" variant="heat" asChild><a href={callHref ?? "#"} aria-disabled={!callHref} onClick={(event) => { if (!callHref) event.preventDefault(); }}><Phone className="size-4" />Call Cathy</a></Button><Button className="min-h-12 w-full rounded-2xl" variant="outline" asChild><a href={smsHref ?? "#"} aria-disabled={!smsHref} onClick={(event) => { if (!smsHref) event.preventDefault(); }}><MessageCircle className="size-4" />Text Cathy</a></Button></div></CardContent></Card>
      ...
      <footer className="mt-6 space-y-3 border-t border-border py-5 text-xs leading-5 text-muted-foreground"><p><strong className="text-foreground">Sources:</strong> Kathy O’Malley’s “a few more to see” email (Aug 24), local purchase context notes, current Zillow/broker listing details, and Scottsdale weather. Email controls the intended tour list; Cathy controls final access and timing.</p></footer>
    </div>
